...
for lang in hy kk af de fr fi hu gl; do
    ...
    #INT_CKPT_DIR="/exp/estengel/miso_res/ud_models_retrain/transformer_pretrained/${lang}_intermediate/"
    ...
    INT_CKPT_DIR="/exp/estengel/miso_res/ud_models_retrain/transformer_pretrained/${lang}_biaffine/"
    INT_CONFIG="miso/training_config/ud_parsing/transformer_pretrained/${lang}_biaffine.jsonnet"
    echo "qsub -v \"CHECKPOINT_DIR=${INT_CKPT_DIR},TRAINING_CONFIG=${INT_CONFIG}\" grid_scripts/decomp.sh "
    qsub -v "CHECKPOINT_DIR=${INT_CKPT_DIR},TRAINING_CONFIG=${INT_CONFIG}" grid_scripts/decomp.sh
