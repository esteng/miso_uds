...
if __name__=="__main__":
    ...
    output_dir = pathlib.Path(sys.argv[1]).parent
    

    with open(output_dir.joinpath("alter_dict.json"),'w') as f:
        f.write(json.dumps(alterations))

    orig_full_gold = compare_all_attachments(trees_orig)
    with open(output_dir.joinpath("orig_gold_dict.json"),'w') as f:
        f.write(json.dumps(orig_full_gold))
        
    edit_full_gold = compare_all_attachments(trees_edit)
    with open(output_dir.joinpath("edit_gold_dict.json"),'w') as f:
