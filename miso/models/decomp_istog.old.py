...
from miso.modules.decoders.attribute_decoder import NodeAttributeDecoder, NodeAttributeDecoderWithSourceAttention
from miso.modules.decoders.deep_biaffine_tree_decoder import DeepBiaffineTreeDecoder
from miso.utils.nn import get_text_field_mask
from miso.utils.string import START_SYMBOL, END_SYMBOL, find_similar_token, is_abstract_token
from miso.data.vocabulary import DEFAULT_OOV_TOKEN, DEFAULT_PADDING_TOKEN
from miso.data.tokenizers.character_tokenizer import CharacterTokenizer
from miso.data.dataset_readers.ontology import NODE_ONTOLOGY, EDGE_ONTOLOGY
...
class ISTOG(Model):
    ...
    def __init__(self,
                 ...
                 encoder_anonym_indicator_embedding,
        ...
        super(ISTOG, self).__init__()

        self.vocab = vocab
        self.punctuation_ids = punctuation_ids
        self.use_anonym_indicator = use_anonym_indicator
        ...
        self.encoder_anonym_indicator_embedding = encoder_anonym_indicator_embedding
        ...
        self.node_attribute_module = node_attribute_module
    ...
    def get_metrics(self, reset: bool = False, mimick_test: bool = False):
        ...
        node_attr_metrics = self.node_attribute_module.metrics.get_metric()
        edge_attr_metrics = self.tree_decoder.edge_attribute_decoder.metrics.get_metric()
        metrics.update(generator_metrics)
        metrics.update(tree_decoder_metrics)
        metrics.update(node_attr_metrics)
        metrics.update(edge_attr_metrics)

        if 'F1' not in metrics:
            metrics['F1'] = metrics['all_acc']
        if "Prsn" not in metrics:
            metrics['Prsn'] = 0.0
        return metrics

    def mimick_test(self, data = None):
        word_splitter = None
        if self.use_bert:
            word_splitter = self.test_config.get('word_splitter', None)
        # set a line limit so that mimick_test does not take forever to load and decode 
        line_limit = self.test_config.get("line_limit", 64)
        dataset_reader = load_dataset_reader('decomp', word_splitter=word_splitter, line_limit = line_limit)
        dataset_reader.set_evaluation()
        predictor = Predictor.by_name('Decomp')(self, dataset_reader)
        try:
            logger.info('Computing the S score...')
            # read, predict, and score
            s_scorer = Scorer(predictor, 
                              self.test_config['data'],
                              self.test_config['batch_size'],
                              silent = True,
                              beam_size = self.test_config['beam_size'], 
                              save_pred_path = None,
                              load_path = None,
                              semantics_only = self.test_config['semantics_only'],
                              drop_syntax = self.test_config['drop_syntax'] ,
                              line_limit = line_limit)

            precision, recall, best_f_score = s_scorer.predict_and_compute()

            # compute with oracle to get average pearson r
            ...
            res_dict = manager.run()
            avg_r_value = pearson_r(res_dict)

            return dict(PREC=precision*100, REC=recall*100, F1=best_f_score*100, Prsn=avg_r_value)

        except Exception as e:
            logger.info('Exception threw out when computing s_score .')
    ...
    def prepare_batch_input(self, batch):
        ...
        encoder_inputs = dict(
            ...
            anonym_indicator=encoder_must_copy_tags,
        ...
        node_attribute_values = batch.get('tgt_attributes', None)
        node_attribute_mask = batch.get('tgt_attributes_mask', None)
        edge_attribute_values = batch.get('edge_attributes', None)
        edge_attribute_mask = batch.get('edge_attributes_mask', None)

        attribute_inputs = dict(
    ...
    def forward(self, batch, for_training=False, oracle = False):
        encoder_inputs, decoder_inputs, generator_inputs, parser_inputs, attribute_inputs = self.prepare_batch_input(batch)
        ...
        if for_training or oracle:
            ...
            if isinstance(self.node_attribute_module, NodeAttributeDecoderWithSourceAttention):
                node_attributes = self.node_attribute_module(
                    decoder_outputs['memory_bank'],
                    encoder_outputs['memory_bank']
                )
            else:
            ...
            node_attribute_loss = self.node_attribute_module.compute_loss(
                    node_attributes['pred_attributes'],
                    node_attributes['pred_mask'],
                    attribute_inputs['node_attribute_truth'],
                    mask = attribute_inputs['node_attribute_mask'],
            ) 
            ...
            tree_decoder_outputs = self.tree_decode(
                ...
                attribute_inputs['edge_attribute_truth'],
                attribute_inputs['edge_attribute_mask']
            )
            if not oracle:

                return dict(
                    loss=generator_loss_output['loss'] + tree_decoder_outputs['loss'] + node_attribute_loss['loss'],
                ...
                # change up the output format, returning just the attributes since the structure is gold 
                return dict(
                        node_attributes=node_attributes['pred_attributes'],
                        edge_attributes = tree_decoder_outputs['edge_attributes']
                        )

        else:
            # not train-time, but test-time 
    ...
    def encode(self, bert_tokens, token_subword_index, tokens, pos_tags, anonym_indicators, chars, mask):
        ...
        if self.use_anonym_indicator:
            anonym_indicator_embeddings = self.encoder_anonym_indicator_embedding(anonym_indicators)
            encoder_inputs += [anonym_indicator_embeddings]
    ...
    def tree_decode(self, memory_bank, edge_heads, edge_labels, edge_mask, node_mask, edge_attributes, edge_attribute_mask):
        # Exclude the BOS symbol.
        node_mask = node_mask.byte()
        memory_bank = memory_bank[:, 1:]
        memory_bank = self.decoder_embedding_dropout(memory_bank)
        return self.tree_decoder.get_loss(
            memory_bank, memory_bank, edge_heads, edge_labels, edge_mask, node_mask, edge_attributes, edge_attribute_mask)
    ...
    def decode(self, input_dict, oracle = False):
        if not oracle:
            ...
            return dict(
                ...
                edge_attributes=generator_outputs['edge_attributes'],
                edge_attributes_mask=generator_outputs['edge_attributes_mask'],
                node_attributes=generator_outputs['node_attributes']['pred_attributes'],
                node_attributes_mask=generator_outputs['node_attributes']['pred_mask'],
                corefs=generator_outputs['coref_indexes'],
            )

        else:
            # decode as while training 
            # already decoded in forward, so just return one level up
            return input_dict
    ...
    def beam_search_with_pointer_generator(
        ...
        beam_buffer["edge_attributes"] = mask.new_full(
            (batch_size, beam_size, self.max_decode_length, len(EDGE_ONTOLOGY)),
            0
        )

        beam_buffer["edge_attributes_mask"] = mask.new_full(
            (batch_size, beam_size, self.max_decode_length, len(EDGE_ONTOLOGY)),
            0
        )
        ...
        for step in range(self.max_decode_length):  # one extra step for EOS marker
            ...
            if step != 0:
                ...
                heads, labels, attributes, attribute_mask = self.tree_decoder(queries, keys, edge_mask)
                ...
                update_tensor_buff(
                    "edge_attributes", step, beam_indices, attributes, False
                    )
                update_tensor_buff(
                    "edge_attributes_mask", step, beam_indices, attribute_mask, False
                    )
                #edge_heads += [heads]
                #edge_labels += [labels]
        ...
        # having done beam search, decode node and edge attributes 
        # This turns out to be easier and not much slower than trying to update these dynamically during beam search
        if isinstance(self.node_attribute_module, NodeAttributeDecoderWithSourceAttention):
            node_attributes = self.node_attribute_module(return_dict['decoder_memory_bank'][:,1:], memory_bank)
        else:
            node_attributes = self.node_attribute_module(return_dict['decoder_memory_bank'][:,1:])

        return_dict["predictions"] = return_dict["predictions"][:, :-1]
        return_dict["predictions"][return_dict["predictions"] == pad_token] = eos_token
        
        return_dict["node_attributes"] = node_attributes
        ...
        queries = return_dict['decoder_rnn_memory_bank'][:,1:]
    
        keys = return_dict["decoder_rnn_memory_bank"][:, 1:]
                    

        edge_attributes, edge_attributes_mask = self.tree_decoder.forward_attribute_only(keys, queries, return_dict['edge_heads'])
        return_dict['edge_attributes'] = edge_attributes
        return_dict['edge_attributes_mask'] = edge_attributes_mask
    ...
    def from_params(cls, vocab, params):
        ...
        try:
            loss_function = LossFunctionDict()[params['node_decoder']['loss_function']]
        except KeyError:
            logger.warn(f"Loss function {params['node_decoder']['loss_function']} not defined. Reverting to MSELoss")
            loss_function = torch.nn.MSELoss()

        if params['node_decoder']['type'] == NodeAttributeDecoderWithSourceAttention:
            node_attribute_module = NodeAttributeDecoderWithSourceAttention(
                    input_dim = params['decoder']['hidden_size'],
                    hidden_dim = params['node_decoder']['hidden_size'],
                    encoder_hidden_dim = params['encoder']['hidden_size'] * 2,
                    output_dim = len(NODE_ONTOLOGY),
                    n_layers = params['node_decoder']['n_layers'],
                    loss_multiplier = params['node_decoder']['loss_multiplier'],
            ...
            node_attribute_module = NodeAttributeDecoder(
                    ...
                    loss_multiplier = params['node_decoder']['loss_multiplier'],
                    loss_function = loss_function,
                    share_networks = params['node_decoder']['share_networks']
            )
