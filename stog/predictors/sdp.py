...
class SDPPredictor(Predictor):
        ...
        for instance, output in zip(instances, _outputs):
            ...
            outputs.append(
                dict(
                    ...
                    annotated_sentence=instance.fields["annotated_sentence"].metadata,
    ...
    def reselve_node_list(self, output):
        ...
        for tgt_index, coref_index in enumerate(output["corefs"]):
            if tgt_index == int(coref_index - 1) and output["nodes_src_indices"][tgt_index] not in src_index_to_node:
                node_list.append(
                    {
                        "nodes" : output["nodes"][tgt_index],
                        "heads" : {output["heads"][tgt_index] : output["head_labels"][tgt_index]},
                        "copy_indicators" : output["copy_indicators"][tgt_index],
                        "tgt_index" : tgt_index,
                ...
                src_index_to_node[output["nodes_src_indices"][tgt_index]] = node_list[-1]
            else:
                if output["nodes_src_indices"][tgt_index] in src_index_to_node:
                    # This means the decoder copy a source token twice and maybe failed to give a correct coref
                    # Consider it as a coref
                    tgt_index_to_node[tgt_index] = node_list[-1]
                    coref_tgt_index = src_index_to_node[output["nodes_src_indices"][tgt_index]]["tgt_index"]
                else:
                    coref_tgt_index = int(coref_index - 1)

                if output["heads"][tgt_index] not in tgt_index_to_node[coref_tgt_index]["heads"]:
        ...
        for node in node_list:
            for head_idx, head_label in node["heads"].items():
                ...
                if "_reversed" in head_label:
                    ...
                    node["pred"] = 1
                    tgt_index_to_node[head_idx - 1]["parents"][node["tgt_index"]] = true_label 
                else:
    ...
    def dump_line(self, output):
        ...
        for src_idx, item in enumerate(output["annotated_sentence"]):
            ...
            if src_idx in src_index_to_node:
                ...
                for pred_idx in range(num_pred):
                    if pred_idx in node["pred_parents"]:
                        new_item["relations"].append(node["pred_parents"][pred_idx])
                    else:
            ...
            string_to_print += "\t".join(value for value in new_item.values()) + '\n'
