...
def character_tensor_from_token_tensor(
        ...
        vocab,
        character_tokenizer,
        namespace=dict(tokens="token_ids", characters="token_characters")
):
...
class STOGSDP(Model):
    ...
    def prepare_batch_input(self, batch):
        ...
        # [batch, num_tokens]
        decoder_token_inputs = batch['tgt_tokens']['tokens'][:, :-1].contiguous()
        if 'tgt_pos_tags' in batch:
            decoder_pos_tags = batch['tgt_pos_tags'][:, :-1]
        else:
            decoder_pos_tags = None
        # [batch, num_tokens, num_chars]
        decoder_char_inputs = batch['tgt_tokens']['characters'][:, :-1].contiguous()
        ...
        # [batch, num_tokens]
        vocab_targets = batch['tgt_tokens']['tokens'][:, 1:].eq(
            self.vocab.get_token_index(END_SYMBOL, "token_ids")
        ).contiguous()
        # [batch, num_tokens]
        ...
        parser_token_inputs[
            parser_token_inputs == self.vocab.get_token_index(END_SYMBOL, 'token_ids')
        ] = 0
    ...
    def beam_search_with_pointer_generator(
        ...
        bos_token = self.vocab.get_token_index(START_SYMBOL, "token_ids")
        eos_token = self.vocab.get_token_index(END_SYMBOL, "token_ids")
        pad_token = self.vocab.get_token_index(DEFAULT_PADDING_TOKEN, "token_ids")
        ...
        for step in range(self.max_decode_length):  # one extra step for EOS marker
            ...
            eos_token_mask = new_token_indices.eq(0)
            ...
            eos_ranks = torch.fmod(torch.arange(eos_token_mask[:, :beam_size].numel()).type_as(eos_token_mask).view(batch_size, -1), beam_size).masked_select(eos_token_mask[:, :beam_size])

            if eos_beam_indices_offset.numel() > 0:
                for index, rank in zip(eos_beam_indices_offset.tolist(), eos_ranks.tolist()):
                    ...
                    if step > 0 and hypo_score > bucket_max_score[eos_batch_idx] and rank == 0:
            ...
            input_tokens, _predictions, pos_tags, corefs, _mask = self._update_maps_and_get_next_input(
                ...
                flatten(beam_buffer["predictions"]),
                generator_output['source_dynamic_vocab_size'],
        ...
        return_dict["decoder_mask"] = return_dict["predictions"] != 0#return_dict["decoder_mask"][:, :-1]
    ...
    def _update_maps_and_get_next_input(
            self, step, predictions, prev_predictions, copy_vocab_size, coref_attention_maps,
        ...
        coref_predictions_in_copy_index = prev_predictions.gather(
            1, 
            ((coref_predictions - 1) * coref_mask.type_as(coref_predictions)).unsqueeze(1)
        ).squeeze(1)

        # prediction in src index
        copy_predictions = (predictions - vocab_size) * copy_mask.long() \
            + (coref_predictions_in_copy_index - vocab_size) * coref_mask.long()

        pos_tags = torch.full_like(predictions, self.vocab.get_token_index(DEFAULT_OOV_TOKEN, 'pos_tags'))
        copy_predictions_in_vocab_index = \
        ...
        for i, index in enumerate(copy_predictions.tolist()):
            ...
            copy_predictions_in_vocab_index[i] = self.vocab.get_token_index(copied_token, 'token_ids')

        next_input = copy_predictions_in_vocab_index

        # 5. Get the mask for the current generation.
        has_eos = torch.zeros_like(copy_mask)
        if len(masks) != 0:
            has_eos = torch.cat(masks, 1).long().sum(1).gt(0)
        mask = next_input.eq(self.vocab.get_token_index(END_SYMBOL, 'token_ids')) | has_eos

        return (next_input.unsqueeze(1),
                copy_predictions.unsqueeze(1),
    ...
    def from_params(cls, vocab, params):
        logger.info('Building the {} Model...'.format(params["model_type"]))
        ...
        decoder_coref_embedding = None
        if params.get('use_coref_embedding', True):
        ...
        # Vocab
        punctuation_ids = []
        oov_id = vocab.get_token_index(DEFAULT_OOV_TOKEN, 'token_ids')
        for c in ',.?!:;"\'-(){}[]':
            c_id = vocab.get_token_index(c, 'token_ids')
            if c_id != oov_id:
